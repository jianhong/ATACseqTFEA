...
provide the possibility of TF can bind to that position. 
...
by detect the difference in TF footprint via enrichment score[@subramanian2005gene], 
...
An other full list of non-redundant TF motifs are also available by downloading
...
peaks <- sapply(pfs, import, format="narrowPeak")
...
for(TF in res$resultsTable$TF[res$resultsTable$adjPval<0.05]){
  bs <- mts[sapply(mts$motif, 
                   function(.ele) TF %in% .ele)]
  bs$score <- unlist(mapply(bs$score, bs$motif, FUN = function(score, motif){
    score[motif==TF][1]
    }))
